...
KEY_PREFIX = "ark" + "-"
HARDCODED_PATTERN = "HARDCODED_API_" + "KEY" + r'\s*=\s*"[^"]+"'
...
def _scan_for_secrets(root: Path) -> list[str]:
    ...
    for path in root.rglob("*"):
        ...
        if re.search(HARDCODED_PATTERN, text):
            findings.append(str(path.relative_to(root)))
    return findings
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create a public release copy without API keys."
    )
    parser.add_argument(
        "--source",
        default=None,
    ...
    print("Verified: no API key or non-empty HARDCODED_API_KEY.")
    return 0
