...
from read_image.errors import WindowsCaptureError, tr
...
def _load_ps(name: str) -> str:
    return (
        resources.files("read_image.assets.windows").joinpath(name).read_text(
            encoding="utf-8"
        )
    )
...
def capture_windows(
    ...
    output_path = _default_capture_dir() if not output_dir else Path(
        output_dir
    ).expanduser().resolve()
    ...
    _run_powershell(_CAPTURE_PS, timeout_sec=90, env=env)
...
def _run_cli() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Capture native Windows screenshots."
    )
    ...
    try:
        if args.list_windows:
            ...
            parser.error("请提供 --list-windows 或 --capture 参数。")
    except WindowsCaptureError as exc:
