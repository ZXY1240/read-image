...
from read_image.errors import ReadImageError, VisionMediaError, tr
...
def _prepare_lossless_image(
    ...
    if image_format_name == "GIF":
        ...
        prepared = prepared.convert(
            "RGBA" if _has_transparency(prepared) else "RGB"
        )
...
def prepare_image(raw_path: str) -> tuple[bytes, str]:
    ...
    if not raw_path:
        raise ReadImageError(
            tr("image 参数为空。", "image argument is empty.")
        )
    ...
    if not path.is_file():
        raise ReadImageError(
            tr(f"找不到图片文件：{path}", f"Image file not found: {path}")
        )
...
def _transcode_video(
    ...
    *,
    ...
    if result.returncode != 0 or not output_path.is_file():
        ...
        raise ReadImageError(
            tr(f"视频转换失败：{detail}", f"Video conversion failed: {detail}")
        )
...
def _download_video_url(
    ...
    try:
        ...
        part.unlink(missing_ok=True)
        raise ReadImageError(
            tr(f"下载视频失败：{exc}", f"Video download failed: {exc}")
        ) from exc
...
def _normalized_error_code(value: str | None) -> str:
...
def _is_file_id_rejection(exc: Exception) -> bool:
    ...
    normalized = _normalized_error_code(error_code)
    if any(
...
def _compress_video_to_limit(
    ...
    *,
    ...
    _transcode_video(input_path, output_720, scale=720, crf=28)
    ...
    _transcode_video(input_path, output_480, scale=480, crf=32)
...
def _delete_uploaded_video_file(file_id: str) -> None:
    try:
        api.delete_video_file(file_id)
    except Exception:
        pass
...
def _analyze_local_video_files(
    ...
    try:
        try:
            return api.call_video_file_id(file_id, task, mode)
        except ReadImageError as exc:
...
def analyze_video(video: str, task: str, mode: str | None) -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="read-image-video-") as tmp:
        ...
        if not path.is_file():
            raise ReadImageError(
                tr(f"找不到视频文件：{path}", f"Video file not found: {path}")
            )
