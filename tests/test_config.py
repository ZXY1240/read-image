...
def test_provider_selection_zai_and_openai(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OMNIMODAL_PROVIDER", "zai")
    monkeypatch.delenv("OMNIMODAL_BASE_URL", raising=False)
    assert provider() == "zai"
    assert provider_name() == "zai"
    ...
    monkeypatch.setenv("OMNIMODAL_PROVIDER", "bogus")
    assert provider() == "dashscope"


def test_provider_specific_generation_models(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OMNIMODAL_PROVIDER", "zai")
    monkeypatch.delenv("OMNIMODAL_BASE_URL", raising=False)
    monkeypatch.delenv("OMNIMODAL_VIDEO_GEN_BASE_URL", raising=False)
    assert image_generation_model("standard") == "glm-image"
    assert video_generation_model("standard") == "cogvideox-3"
    assert video_generation_base_url() == "https://api.z.ai/api/paas/v4"
    monkeypatch.setenv("OMNIMODAL_VIDEO_GEN_BASE_URL", "https://video.example.invalid")
    assert video_generation_base_url() == "https://video.example.invalid"


def test_openai_compatible_video_generation_raises(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(ReadImageError):
        video_generation_model("standard")
