...
- 新增多厂商兼容：`OMNIMODAL_PROVIDER=dashscope|zai|openai_compatible`，默认仍为 DashScope。
- Z.AI/GLM 支持图片/视频/音频理解、GLM 文生图、GLM 文生视频（异步轮询）和 GLM ASR。
- OpenAI 兼容 Provider 支持识别与 OpenAI Images 风格文生图/图片编辑；视频/音频生成无统一标准时返回清晰错误。
- `GenerationClient` 泛化异步任务协议，支持 DashScope 与 Z.AI 的不同提交字段、状态字段和结果字段。
- 新增 `OMNIMODAL_GENERATION_MODEL`、`OMNIMODAL_VIDEO_GEN_BASE_URL`、`OMNIMODAL_AUDIO_GEN_BASE_URL` 等配置。
- README 增加 Demo / 效果展示，新增本地生成的 `docs/demo-source.png` 和真实输出摘要 `docs/demo-output.txt`。
