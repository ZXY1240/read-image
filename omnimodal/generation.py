...
class GenerationSpec:
    ...
    price_unit: str = ""
    # Provider-specific async result conventions.
    use_dashscope_async_header: bool = True
    submit_result_path: tuple[str, ...] = ("output", "task_id")
    poll_url_template: str | None = None
    status_path: tuple[str, ...] = ("output", "task_status")
    success_statuses: tuple[str, ...] = ("SUCCEEDED",)
    failure_statuses: tuple[str, ...] = ("FAILED", "CANCELED")
...
class GenerationClient:
    ...
    def submit(self, payload: dict[str, Any]) -> str:
        ...
        headers = _auth_headers(self.extra_headers)
        if self.spec.use_dashscope_async_header:
        ...
        task_id: Any = parsed
        for key in self.spec.submit_result_path:
    ...
    def poll_status(self, task_id: str) -> dict[str, Any]:
        ...
        仅 5xx / 网络错误 / 429 重试。
        """
        if self.spec.poll_url_template:
            url = self.spec.poll_url_template.format(task_id=task_id)
        else:
    ...
    def wait_for_result(
        ...
        while time.monotonic() < deadline:
            ...
            status_node: Any = data
            for key in self.spec.status_path:
                if not isinstance(status_node, dict):
                    status_node = None
                    break
                status_node = status_node.get(key)
            status = status_node
            if status in self.spec.success_statuses:
                return data
            if status in self.spec.failure_statuses:
                if self.spec.status_path == ("task_status",):
                    detail = data.get("message") or data.get("error") or ""
                    error_code = data.get("code") or data.get("error_code") or ""
                else:
