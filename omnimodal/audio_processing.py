...
def _transcribe_openai_like_audio(
    path_or_url: str,
    language: str = "zh",
    model: str | None = None,
) -> dict[str, Any]:
    """Transcribe with Z.AI/OpenAI-style multipart /audio/transcriptions."""
    selected_model = model or asr_model_name()
    ...
    if path_or_url.startswith(("http://", "https://")):
        ...
        tmp = tempfile.TemporaryDirectory(prefix="omnimodal-asr-")
        tmp_dir = Path(tmp.name)
        local_path = tmp_dir / "audio"
        try:
            with http_client.stream("GET", path_or_url, timeout=120.0) as response:
                if response.status_code >= 400:
                    raise ReadImageError(
                        tr(
                            "下载待转写音频失败。",
    ...
    mime = _mime_for_audio(str(local_path))
    data: dict[str, Any] = {"model": selected_model, "stream": "false"}
    if language:
    ...
    try:
        ...
        raise ReadImageError(
            tr(
                "语音转写请求失败（网络错误）。",
                "Transcription request failed (network error).",
            )
        ) from exc
    finally:
    ...
    if response.status_code >= 400:
        raise ReadImageError(
            tr(
                f"语音转写请求失败（HTTP {response.status_code}）。",
                f"Transcription request failed (HTTP {response.status_code}).",
            )
            + f" {response.text[:300]}"
        )
    try:
...
def transcribe_audio(
    ...
    """
    selected_model = model or asr_model_name()
    if provider() in {"zai", "openai_compatible"} or selected_model.lower().startswith(
        ("glm-asr", "whisper")
    ):
        return _transcribe_openai_like_audio(
            path_or_url,
            language=language,
            model=selected_model,
        )
...
def recognize_audio(
    ...
    if provider() in {"zai", "openai_compatible"}:
        from omnimodal import api

        return api.default_client.provider.call_audio(
            path_or_url,
            task,
            mode,
        )
