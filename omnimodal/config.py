...
def base_url() -> str:
    return env("OMNIMODAL_BASE_URL", DEFAULT_BASE_URL).rstrip("/")


def image_model() -> str:
    return env("OMNIMODAL_IMAGE_MODEL", DEFAULT_IMAGE_MODEL)
...
def video_model() -> str:
    return env("OMNIMODAL_VIDEO_MODEL", DEFAULT_VIDEO_MODEL)


def audio_model(tier: str | None = None) -> str:
    name = tier if tier in {"pro", "max"} else "standard"
    return env(
        f"OMNIMODAL_AUDIO_MODEL_{name.upper()}",
        DEFAULT_AUDIO_MODEL if name == "standard" else "qwen3.5-omni-plus",
    )


def ocr_model_name() -> str:
    return env("OMNIMODAL_OCR_MODEL", DEFAULT_OCR_MODEL)


def asr_model_name() -> str:
    return env("OMNIMODAL_ASR_MODEL", DEFAULT_ASR_MODEL)
