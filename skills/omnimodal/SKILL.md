---
name: omnimodal
description: 为 DeepSeek 等纯文本主模型提供多模态能力（看图片/视频、听音频、识别语音、生成图片/视频/语音）。遇到图片路径、截图、UI/网页截图、图表、错误弹窗、OCR/识别/提取图中文字、音频/视频内容理解、文生图/文生视频/TTS 或任何需要多模态能力的任务时自动启用。
---
# Omnimodal
...
`read_image` 只读取用户明确要求或任务明确需要的图片文件。不要读取密码、密钥、认证材料等敏感文件。
...
- `ocr`：文字提取（OCR 专用模型 qwen-vl-ocr，比通用视觉便宜）。
...
- 用户说"高质量/精致/专业/商用/精美" → `pro`（更强模型）
- 用户说"电影级/顶级/旗舰/大片/震撼" → `max`（最强模型，仅视频有）
- `READ_IMAGE_DEFAULT_TIER` 可调全局默认档位

**生成结果**自动保存到输出目录并回读验证，返回文件路径。视频时长受 `READ_IMAGE_MAX_VIDEO_DURATION`（默认 15s）限制。
...
API Key：`READ_IMAGE_API_KEY`。**完整功能（音频识别 + 生成能力）建议在千问 AI 平台（qianwenai.com）申请 API key**（`sk-ws-` 开头，可直接调用全套模型）；仅图片/视频识别可用百炼等平台的 `sk-` key，但生成/音频等能力取决于 key 开通范围。也兼容 `ARK_API_KEY`、`DOUBAO_API_KEY`、`VISION_API_KEY`。

推荐在插件根目录创建本地 `.env`：

```powershell
READ_IMAGE_API_KEY=你的千问AI平台或DashScope API Key
...
- `READ_DRAG_DIRS`
- `WINDOWS_CAPTURE_DIR`
- `READ_IMAGE_DEFAULT_TIER`（standard/pro/max，生成与音频的默认档位）
- `READ_IMAGE_GENERATION_OUTPUT_DIR`（生成结果输出目录，须在白名单内）
- `READ_IMAGE_GENERATION_TIMEOUT_SEC`（文生图/语音超时，默认 300）
- `READ_VIDEO_GENERATION_TIMEOUT_SEC`（文生视频超时，默认 900）
- `READ_IMAGE_MAX_VIDEO_DURATION`（文生视频时长上限，默认 15s）
- `READ_IMAGE_TTS_VOICE`（默认音色 longxiaochun_v2）
- `READ_IMAGE_CACHE_DIR`（磁盘缓存目录，默认空=不持久化）
